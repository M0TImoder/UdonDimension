...
impl Config {
    ...
    pub fn new<P: AsRef<Path>>(path: P) -> Config
    {
        ...
        config.apply_policy_args_from_env();

        config
    }

    fn apply_policy_args_from_env(&mut self)
    {
        self.append_policy_arg("CMAKE_POLICY_VERSION");
        self.append_policy_arg("CMAKE_POLICY_VERSION_MINIMUM");
    }

    fn append_policy_arg(&mut self, key: &str)
    {
        if let Some(value) = env::var_os(key)
        {
            let mut arg = OsString::from("-D");
            arg.push(OsStr::new(key));
            arg.push(OsStr::new("="));
            arg.push(value.as_os_str());
            self.configure_args.push(arg);
        }
    }
